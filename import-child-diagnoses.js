...
const childDiagnosesData = {
  ...
  "emotional_psychological": {
    ...
    diagnoses: [
      ...
      {
        id: "behavioral_regulation_challenges",
...
async function importChildDiagnoses() {
  try {
    ...
    for (const [categoryId, categoryData] of Object.entries(childDiagnosesData)) {
      ...
      // Add each diagnosis as a subcollection document
      for (const diagnosis of categoryData.diagnoses) {
        const diagnosisRef = categoryRef.collection('diagnoses').doc(diagnosis.id);
        const diagnosisDoc = {
          name: diagnosis.name,
        ...
        batch.set(diagnosisRef, diagnosisDoc);
        totalDiagnoses++;
      }
    }
