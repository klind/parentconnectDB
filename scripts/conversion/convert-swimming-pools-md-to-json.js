...
console.log("🔄 Converting Swimming Pools.md to JSON format:");
console.log("================================================");
console.log("");

// Read the markdown file
const markdownContent = fs.readFileSync('./SaltLakeCityResources/Swimming Pools.md', 'utf8');

// Parse the markdown content into structured data
function parseSwimmingPoolsMarkdown(content) {
  const resources = [];
...
const swimmingPoolsData = parseSwimmingPoolsMarkdown(markdownContent);

// Create the JSON structure - simplified without metadata
const jsonOutput = {
  category: "Swimming Pools",
  totalResources: swimmingPoolsData.length,
  resources: swimmingPoolsData
};

// Write to JSON file
const outputPath = './SaltLakeCityResources/swimming-pools.json';
fs.writeFileSync(outputPath, JSON.stringify(jsonOutput, null, 2));

console.log(`✅ Successfully converted ${swimmingPoolsData.length} swimming pool resources to JSON`);
...
swimmingPoolsData.forEach((resource, index) => {
