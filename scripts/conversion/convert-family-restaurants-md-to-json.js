...
console.log("🔄 Converting Family Restaurants.md to JSON format:");
console.log("===================================================");
console.log("");

// Read the markdown file
const markdownContent = fs.readFileSync('./SaltLakeCityResources/Family Restaurants.md', 'utf8');

// Parse the markdown content into structured data
function parseFamilyRestaurantsMarkdown(content) {
  const resources = [];
...
const familyRestaurantsData = parseFamilyRestaurantsMarkdown(markdownContent);

// Create the JSON structure - simplified without metadata
const jsonOutput = {
  category: "Family Restaurants",
  totalResources: familyRestaurantsData.length,
  resources: familyRestaurantsData
};

// Write to JSON file
const outputPath = './SaltLakeCityResources/family-restaurants.json';
fs.writeFileSync(outputPath, JSON.stringify(jsonOutput, null, 2));

console.log(`✅ Successfully converted ${familyRestaurantsData.length} family restaurant resources to JSON`);
...
familyRestaurantsData.forEach((resource, index) => {
