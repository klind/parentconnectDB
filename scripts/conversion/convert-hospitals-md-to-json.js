...
console.log("🔄 Converting Hospitals.md to JSON format:");
console.log("===========================================");
console.log("");

// Read the markdown file
const markdownContent = fs.readFileSync('./SaltLakeCityResources/Hospitals.md', 'utf8');

// Parse the markdown content into structured data
function parseHospitalsMarkdown(content) {
  const resources = [];
...
const hospitalsData = parseHospitalsMarkdown(markdownContent);

// Create the JSON structure - simplified without metadata
const jsonOutput = {
  category: "Hospitals",
  totalResources: hospitalsData.length,
  resources: hospitalsData
};

// Write to JSON file
const outputPath = './SaltLakeCityResources/hospitals.json';
fs.writeFileSync(outputPath, JSON.stringify(jsonOutput, null, 2));

console.log(`✅ Successfully converted ${hospitalsData.length} hospital resources to JSON`);
...
hospitalsData.forEach((resource, index) => {
