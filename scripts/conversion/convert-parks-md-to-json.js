...
console.log("🔄 Converting Parks.md to JSON format:");
console.log("=======================================");
console.log("");

// Read the markdown file
const markdownContent = fs.readFileSync('./SaltLakeCityResources/Parks.md', 'utf8');

// Parse the markdown content into structured data
function parseParksMarkdown(content) {
  const resources = [];
...
const parksData = parseParksMarkdown(markdownContent);

// Create the JSON structure - simplified without metadata
const jsonOutput = {
  category: "Parks",
  totalResources: parksData.length,
  resources: parksData
};

// Write to JSON file
const outputPath = './SaltLakeCityResources/parks.json';
fs.writeFileSync(outputPath, JSON.stringify(jsonOutput, null, 2));

console.log(`✅ Successfully converted ${parksData.length} park resources to JSON`);
...
parksData.forEach((resource, index) => {
