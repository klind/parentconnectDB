...
console.log("🔄 Converting Shopping.md to JSON format:");
console.log("==========================================");
console.log("");

// Read the markdown file
const markdownContent = fs.readFileSync('./SaltLakeCityResources/Shopping.md', 'utf8');

// Parse the markdown content into structured data
function parseShoppingMarkdown(content) {
  const resources = [];
...
const shoppingData = parseShoppingMarkdown(markdownContent);

// Create the JSON structure - simplified without metadata
const jsonOutput = {
  category: "Shopping",
  totalResources: shoppingData.length,
  resources: shoppingData
};

// Write to JSON file
const outputPath = './SaltLakeCityResources/shopping.json';
fs.writeFileSync(outputPath, JSON.stringify(jsonOutput, null, 2));

console.log(`✅ Successfully converted ${shoppingData.length} shopping resources to JSON`);
...
shoppingData.forEach((resource, index) => {
