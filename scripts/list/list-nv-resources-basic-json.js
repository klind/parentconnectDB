...
async function exportNVResources() {
  try {
    ...
    snapshot.forEach((doc) => {
      ...
      // Only include resources where state = NV
      if (state === "NV") {
        results.push({ name, street, city, state, zip, imageFolder });
      }
    });

    console.log(`Found ${results.length} Nevada resources:`);
    console.log(JSON.stringify(results, null, 2));
  } catch (err) {
    console.error("Error exporting Nevada resources:", err && err.message ? err.message : err);
    process.exitCode = 1;
  }
}
